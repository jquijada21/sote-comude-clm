...
export function OrganigramaTerritorialVertical({
  ...
  const handleCopiarImagen = async () => {
    try {
      ...
      setCopiado(true);
      toast.success("Imagen copiada / descargada.");
  ...
  const exportOptions = [
    {
      id: "pdf",
      label: "Documento PDF",
      description: "Descarga el mapa territorial en formato PDF.",
      icon: organigramaExportIcons.pdf,
      iconClass: "text-red-500",
      onSelect: () => exportTerritorialToPdf("territorial-d3-full-container"),
    },
    {
      id: "png",
      label: "Imagen PNG",
      description: "Descarga una imagen PNG de alta resolución.",
      icon: organigramaExportIcons.image,
      iconClass: "text-emerald-500",
      onSelect: () => exportTerritorialToPng("territorial-d3-full-container"),
    },
  ];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[200] flex flex-col bg-background text-foreground animate-in fade-in duration-200">
      {/* Top Header Bar */}
      <header className="flex shrink-0 items-center justify-between border-b border-border/50 bg-card/80 px-4 py-3 backdrop-blur-md md:px-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] font-medium text-muted-foreground">
              Diagrama jerárquico visual de microrregiones, aldeas y caseríos
            </p>
          </div>
        </div>
      ...
      <div id="territorial-d3-full-container" className="org-canvas org-canvas--full relative flex-1 overflow-hidden bg-zinc-900/5 dark:bg-zinc-950">
        ...
        <div className="absolute top-4 right-4 z-20 flex items-center gap-1 rounded-xl border border-border/60 bg-card/90 p-1.5 shadow-md backdrop-blur-md">
          <button
            type="button"
        ...
        <div className="pointer-events-none absolute bottom-4 left-1/2 -translate-x-1/2 rounded-full border border-border/50 bg-card/80 px-4 py-1.5 text-[11px] font-semibold text-muted-foreground shadow-md backdrop-blur-md">
